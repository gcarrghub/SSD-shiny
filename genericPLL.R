genericPLL <- function(
  ...
  cex.in = 1.2){
     ...
     par(cex.lab=cex.in, cex.axis=cex.in, cex.main=cex.in, cex.sub=cex.in)
  ...
  if(doPlots){
       ...
       png(file="temp/displayplot.png", width=600, height=400)
       ...
       if( logTransform)points(x=10^pllData,y=order(pllData)/(max(order(pllData))+1),col='gray',pch=16)
    ...
    if( logTransform){
      ...
      axis(side=1,at=10^as.vector(sapply(ceiling(par("usr")[1]):(ceiling(par("usr")[2])-1),FUN=function(x)x+log10(1:9))),
           tck=-.01,labels=F)
      axis(side=2)
      lines(y=dFUN(xVals,MLE.pars[1],MLE.pars[2]),x=10^xVals,lwd=2)
      lines(y=dFUN(xVals,ciParms05.LCL[1],ciParms05.LCL[2]),x=10^xVals,col="cyan",lwd=2)
      lines(y=dFUN(xVals,ciParms05.UCL[1],ciParms05.UCL[2]),x=10^xVals,col="cyan",lwd=2,lty=2)
      rug(side=1,x=10^pllData,lwd=2,col=rgb(0,0,0,.2))
